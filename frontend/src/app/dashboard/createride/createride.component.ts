...
export class CreaterideComponent {
  ...
  getcountryCode() {
    this.createride.getcode().subscribe({
      next: (response) => {
  ...
  //--------------- TO GET NO. OF STOPS ------------------
  getNumberOfStops() {
    this.setting.getsettingdata().subscribe({
      next: (response: any) => {
        console.log(response);
        this.stops = response.stopCounts;
  ...
  getUserDetails() {
    ...
    this.createride.getUserByNumber(userData).subscribe({
      next: (userResponse: any) => {
        console.log(userResponse.user.username)
        this.userArray = userResponse
        this.isUser = true;
        this.polygonsArray = [];  // to clear the polygon array

        this.city.getcity(this.page, this.limit).subscribe({
          next: (resp: any) => {
            ...
            console.log(this.polygonObjects)
          },
          error: (error: any) => {
            console.log(error);
  ...
  startInputChange() {
    ...
    this.startLocation = (document.getElementById("startInput") as HTMLInputElement).value;
    this.isInZone = true;
          ...
          (results: any, status: any) => {
            if (status === "OK") {
              ...
              if (!this.isInZone) {
                ...
                this.toastr1.showWarning("Service is Not Available in City");
              } else {
                if (this.endInput) {
                ...
                this.toastr1.showSuccess("Service is Available in City");
              }
            } else {
              console.log("Select location from auto suggestion", status);
              this.toastr1.showInfo("Select Location from Auto Suggestion");
            }
          }
        );
      }, 200);
  ...
  calculateRoute() {
    ...
    this.startLocation = (document.getElementById("startInput") as HTMLInputElement).value;
    this.endLocation = (document.getElementById("endInput") as HTMLInputElement).value;
    this.rideForm.reset();
    this.rideForm.reset();
    this.rideForm.patchValue({
      serviceType: "",
      ...
      (response: any, status: string) => {
        if (status === "OK") {
          ...
          if (status === "ZERO_RESULTS") {
            this.isRoute = false;
            this.toastr1.showWarning("No route found for given locations!");
            console.log("No route found for given locations! " + status)
          } else {
            this.isRoute = false;
            this.toastr1.showWarning("Select location from auto suggestion ");
            console.log("Select location from auto suggestion " + status)
          }
        }
      }
  ...
  getVehiclePricing(index: any) {
    ...
    this.createride.getServiceType(cityId).subscribe({
  ...
  handleRadioChange() {
    ...
    if (this.selectedOption === "bookNow") {
      ...
      if (!this.isDateTimeValid) {
        this.toastr1.showInfo("Please select valid date and time");
      }
  ...
  onBookRide() {
    ...
    this.rideData = {
      ...
      userId: this.userArray.user._id,
    ...
    this.createride.addRide(this.rideData).subscribe({
      next: (response) => {
        // console.log(response);

        this.toastr1.showSuccess("Ride booked successfully!");
      ...
      error: (error) => {
        console.log(error);
        // this.toastr1.error(error.message);
      },
    });
  }
